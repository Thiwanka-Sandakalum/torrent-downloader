...
func SetupRouter(router *gin.Engine) {
	...
	torrentGroup := router.Group("/torrent")
	{
		...
		torrentGroup.GET("/:taskID/status", torrent.GetTorrentStatus)
		torrentGroup.POST("/:taskID/pause", func(c *gin.Context) {
			torrent.PauseTorrent(c)
			c.Status(200)
		})
		torrentGroup.POST("/:taskID/resume", func(c *gin.Context) {
